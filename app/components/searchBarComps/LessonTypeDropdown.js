...
const LessonTypeDropdown = ({ isVisible, onClose, lessonTypes }) => {
    ...
    return (
        isVisible && (
            <div
                ref={divRef}
                className="absolute flex flex-col left-1/2 top-full mt-2 py-2 
            ...
            >
                ...
                rounded-xl whitespace-nowrap hover:bg-gray-200">
                    <p className='text-[16px] font-semibold'>{item.lessonType}</p>
                    <p className='text-[14px] text-graySubtitle'>{item.lessonTypeDescription}</p>
                </div>
                ))}
                </div>
            </div>
